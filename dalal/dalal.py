...
class Dalal:
    ...
    def gainers(self, index: str | None = None, *, exchange: str = "NSE") -> list[dict]:
        exchange = self._validate_exchange(exchange)
        if exchange == "NSE":
            return self._get_nse().gainers(index or "NIFTY 50")
        return self._get_bse().gainers()

    def losers(self, index: str | None = None, *, exchange: str = "NSE") -> list[dict]:
        exchange = self._validate_exchange(exchange)
        if exchange == "NSE":
    ...
    def status(self, *, exchange: str = "NSE") -> list[dict]:
        exchange = self._validate_exchange(exchange)
        if exchange == "NSE":
            return self._get_nse().status()
        return self._get_bse().status()

    def lookup(self, query: str, *, exchange: str = "NSE") -> list[dict]:
        exchange = self._validate_exchange(exchange)
        if exchange == "NSE":
